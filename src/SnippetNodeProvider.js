...
class SnippetNodeProvider {
    ...
    async editSnippet(e, def) {
        ...
        await editor.edit((eb) => {
            ...
            let comment = utils.getLineComment(e.languageId);
            let text = this.snippet2text(snippet, comment);
    ...
    snippet2text(snippet, comment) {
        let text = "";
        for (let k in snippet) {
            if (k == "body") continue;
            let v = snippet[k];
    ...
    saveSnippet(languageId, key, text) {
        ...
        let snippet = this.text2snippet(text, comment);
        if (!snippet.prefix)
